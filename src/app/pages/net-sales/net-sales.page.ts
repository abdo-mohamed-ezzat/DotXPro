...
export class NetSalesPage {
  ...
  onTypeOfItemChanged($event: IonRadioGroupCustomEvent<RadioGroupChangeEventDetail<any>>){
  ...
  onTypeOfReportChanged($event: IonRadioGroupCustomEvent<RadioGroupChangeEventDetail<any>>){
  ...
  filterPaymentType(name: string): IPaymentMethod[] {
    ...
    return res.length ? res : [{ name: 'No name found', id: 0 }];
  }
  // set emited values
