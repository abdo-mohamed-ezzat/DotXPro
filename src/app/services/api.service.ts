...
export class APIService {
  constructor(private http: HttpClient, private datePipe: DatePipe, private alertController: AlertController) {}
  ...
  private apiCall<T>(endpoint: string, params?: any): Observable<any> {
    ...
    return this.http
      .get<IAPIResponse<T>>(`${environment.APIURL}/${endpoint}`, { params: httpParams })
      .pipe(
        map((res: any) => {
          if (res.success) {
            return res.data;
  ...
  getAccountsLevels(): Observable<ILevel[]> { 
    const endpoint = 'CommonLists/GetAccountsLevel';
    return this.apiCall(endpoint);
  }
}
